...
class EndOfRunMessage {
    ...
    printCacheHitsMessage() {
        ...
        const message = [];
        if (hits.length > 0) {
            message.push(`${chalk.white('Nx Cloud')} made it possible to reuse ${tasks}${this.runContext.runUrl
                ? `: ${chalk.white(this.runContext.runUrl)}`
                : ''}`);
        }
        else {
            if (this.runContext.runUrl) {
                message.push(`See ${chalk.white('Nx Cloud')} run details at ${chalk.white(this.runContext.runUrl)}`);
            }
        }
        if (message.length > 0) {
